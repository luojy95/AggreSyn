...
from scripts.gradation_helper import example_gradations
...
parser.add_argument("--render_multi_light", action="store_true", default=False)
...
# MESH_PATH = C.ROCK_MESH_PATH #C.ROCK_MESH_PATH
MESH_PATH = C.HHH_MESH_PATH #C.ROCK_MESH_PATH
...
BASE_LANDSCAPE_Z = 0.01 * SCENE_SIZE
...
CAMERA_TRAJECTORY_START = np.array([0, -SCENE_SIZE/4, args.camera_height * GLOBAL_SCALE])
CAMERA_TRAJECTORY_END = np.array([0, SCENE_SIZE/4, args.camera_height * GLOBAL_SCALE])
...
if __name__=='__main__':
    ...
    base_landscape_material = M.load_material(name=fine_grain_material_name, path=fine_grain_material_path, is_ballast=True)
    
    if args.use_mixed_mats:
        ballast_materials = [M.load_mixed_ballast_mat(name=os.path.split(path)[-1], ballast_mat_path=path, fine_mat_path=fine_grain_material_path)
    ...
    bpy.ops.mesh.primitive_cube_add(size=SCENE_SIZE, scale=(1, 1, 0.1), location=(0, 0, -0.05 * SCENE_SIZE))
    base_landscape = bpy.context.object

    # create rigid body
    U.set_active(base_landscape)
    bpy.ops.rigidbody.object_add(type='PASSIVE')
    bpy.ops.rigidbody.shape_change(type='BOX')
    
    M.assign_material(base_landscape, base_landscape_material)
    ...
    # generate ballast
    # ballast_meshes = glob.glob(os.path.join(C.HHH_MESH_DIR, "RR3*.obj"))
    ballast_meshes = glob.glob(MESH_PATH)
    ...
    zs = []
    for ballast in ballasts:
        zs.append(ballast.location[2])
    
    upper_z = np.percentile(zs, 90)
    print(upper_z)
    vb = upper_z * SCENE_SIZE * SCENE_SIZE * (MU.meter2inch(1.0 / GLOBAL_SCALE) ** 3)
    
    print("!!!!!!!!!! Calculate Ballast Density !!!!!!!!!!!")
    print(vb, input_gradation.total_ballast_vol)
    print(input_gradation.total_ballast_vol / vb)
    ...
    while bin_search_iters < DISPLACEMENT_MAX_BIN_SEARCH_ITER and volume_err > DISPLACEMENT_VOL_TOLERANCE:
        ...
        current_vol = G.calc_fine_volume(
            ...
            # base_obj=base_landscape_for_height_calc,
            base_obj=base,
            ballasts_obj=coarse_group,
